namespace CyberDuel.Systems
{
    public class FinanceServer
    {
        ...
        // Port → (servis adı, durum: open/closed/filtered)
        ...
        public int MaxCapacity = 800; // req/s

        // Dosya yolu → erişim için gereken rol
        public Dictionary<string, string> FileSystem = new()
        {
            { "/public/index.html",           "guest" },
            { "/public/assets/logo.png",      "guest" },
            { "/app/dashboard.html",          "user"  },
            { "/var/log/transactions.log",    "user"  },
            { "/admin/config.xml",            "admin" },
            { "/admin/users.db",              "admin" },
            { "/db/customers.db",             "admin" },
            { "/db/credit_cards.db",          "admin" },
            { "/etc/shadow",                  "root"  },
            { "/etc/passwd",                  "root"  }
        };
        ...
        {
            return "MISSION: Breach the Finance Server.\n" +
